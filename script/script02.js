...
const setupEventListeners = () => {
    if (window.innerWidth <= 790) {
        const leftSection = document.querySelector(".left");
        leftSection.removeEventListener("mouseenter", circleEnter);
        leftSection.removeEventListener("mousemove", circleMove);
        leftSection.removeEventListener("mouseleave", circleLeave);

        addClickListeners();
    } else {
        circle.removeEventListener("click", toggleCircle);
        addHoverListeners();
    }
};
